...
async function main() {
  ...
  console.log(
    `Executing borrow_v2 with amount: ${amount}, coin: ${coinSymbol}`
  );
  ...
  console.log("Requesting borrow ticket...");
  ...
  const ticketString = new aptos.HexString(borrowTicket.packet);
  const ticketUintArray = ticketString.toUint8Array();
  const borrowIX = sdk.borrowV2Ix(ticketUintArray, coinType);
  await signSubmitWait(aptosClient, walletAccount, borrowIX);

  console.log("Borrow_v2 transaction completed successfully!");
}
